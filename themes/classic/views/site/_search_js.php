...
function prepareSearch() {
    ...
    if (term != <?php echo CJavaScript::encode(tc("Search by description or address")) ?>) {
        if (term.length >= <?php echo (int) param('minLengthSearch', 4) ?>) {
            term = term.split(" ");
            term = term.join("+");
            $("#do-term-search").val(1);
                window.location.replace("<?php echo Yii::app()->createAbsoluteUrl('/quicksearch/main/mainsearch') ?>?term="+term+"&do-term-search=1");
            } else {
                alert(<?php echo CJavaScript::encode(Yii::t('common', 'Minimum {min} characters.', array('{min}' => param('minLengthSearch', 4)))) ?>);
        }
    }
}