...
class Apartment extends ParentModel
{
    ...
    public function rules()
    {
        ...
        $rules = array(
            ...
            array('in_currency, owner_active, num_of_rooms, is_special_offer, is_free_to, active, metroStations, note, period_activity, date_manual_updated, parent_id_autocomplete', 'safe'),
    ...
    public function afterSave()
    {
        ...
        if ($this->deleted == 0 && issetModule('socialposting') && $this->active == self::STATUS_ACTIVE && $this->owner_active == self::STATUS_ACTIVE) {
        ...
        if (issetModule('seasonalprices') && $this->type == Apartment::TYPE_RENT) {
            ...
            if (!empty($result)) {
                foreach ($result as $key => &$res) {
                    ...
                    $res['date_created'] = new CDbExpression('NOW()');
                }
    ...
    public static function getObjTypesArray($with_all = false)
    {
        ...
        foreach ($objTypeModel as $type) {
            if (in_array($type->id, $excludeList))
                continue;
    ...
    {
        if ($this->active != Apartment::STATUS_DRAFT && $this->type == Apartment::TYPE_RENT && !$this->deleted) {
