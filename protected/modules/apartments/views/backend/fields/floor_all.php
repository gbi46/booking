<?php if ($model->canShowInForm('floor_all')) { ?>
    <div class="form-group">
        ...
        <?php
        echo CHtml::activeDropDownList($model, 'floor', array_merge(
                array('0' => ''), range(1, param('moduleApartments_maxFloor', 30))
            ), array('class' => 'width70 form-control noblock'));

        ?> /
        <?php
        echo CHtml::activeDropDownList($model, 'floor_total', array_merge(
                array('0' => ''), range(1, param('moduleApartments_maxFloor', 30))
            ), array('class' => 'width70 form-control noblock'));
