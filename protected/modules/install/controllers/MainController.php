...
class MainController extends CController
{
    ...
    protected function beforeAction($action)
    {
        ...
        if (!isset(Yii::app()->request->cookies['_lang'])) {
            $tmpLang = 'en';
            Yii::app()->setLanguage($tmpLang);

            if (!empty($_SERVER) && isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
                $tmpLang = substr($_SERVER['HTTP_ACCEPT_LANGUAGE'], 0, 2);

                if (in_array($tmpLang, InstallForm::$_activeLangs)) {
                    $_GET['lang'] = $tmpLang;
                    Yii::app()->setLanguage($tmpLang);
                    $this->setLangCookieInstall($tmpLang);
                }
            }
        }
    ...
    public function actionFinalRequest()
    {
        ...
        if (isset($_SERVER['HTTP_HOST']) || isset($_SERVER['REQUEST_URI'])) {
            if (isset($_SERVER['HTTP_HOST']) && isset($_SERVER['REQUEST_URI'])) {
                getRemoteDataInfo('https://demo-free-ru.open-real-estate.info/license.php?host=' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']);
            } elseif (isset($_SERVER['HTTP_HOST'])) {
                getRemoteDataInfo('https://demo-free-ru.open-real-estate.info/license.php?host=' . $_SERVER['HTTP_HOST']);
            }
        }
    ...
    public function checkRequirements()
    {
        ...
        $requirements = array(
            ...
            array(
                Yii::t('yii', 'MBstring extension'),
                true,
                extension_loaded("mbstring"),
                '<a href="http://php.net/manual/en/mbstring.installation.php">mbstring</a>',
                ''),
            array(
    ...
    function checkRightFile()
    {
        ...
        if (!isFree()) {
            $aCheckDir = array(
                ROOT_PATH . $ds . 'assets',
                ROOT_PATH . $ds . 'protected' . $ds . '_ore_backups',
                ROOT_PATH . $ds . 'protected' . $ds . 'config' . $ds . 'db.php',
                ROOT_PATH . $ds . 'protected' . $ds . 'runtime',
                ROOT_PATH . $ds . 'protected' . $ds . 'runtime' . $ds . 'cache',
                ...
                ROOT_PATH . $ds . 'uploads' . $ds . 'guestad',
                ROOT_PATH . $ds . 'uploads' . $ds . 'editor' . $ds . '.thumbs' . $ds . 'files',
                ROOT_PATH . $ds . 'uploads' . $ds . 'editor' . $ds . '.thumbs' . $ds . 'flash',
                ROOT_PATH . $ds . 'uploads' . $ds . 'editor' . $ds . '.thumbs' . $ds . 'images',
                ROOT_PATH . $ds . 'uploads' . $ds . 'editor' . $ds . 'files',
                ROOT_PATH . $ds . 'uploads' . $ds . 'editor' . $ds . 'flash',
                ROOT_PATH . $ds . 'uploads' . $ds . 'editor' . $ds . 'images',
                ROOT_PATH . $ds . 'uploads' . $ds . 'document',
            );
        } else {
            $aCheckDir = array(
                ROOT_PATH . $ds . 'assets',
    ...
    public function setLangInstall()
    {
        ...
        $lang = 'en';
        $activeLangs = InstallForm::$_activeLangs;
