...
class NewsProduct extends CActiveRecord
{
    ...
    const NEWS_UPDATE_TIME = 172800; //Product news update interval 2 days
    ...
    public static function getFeed($url)
    {
        ...
        $result = getRemoteDataInfo($url, CURLINFO_REDIRECT_URL);

        if (isset($result['answer']) && $result['answer'] != $url) {
            $result = getRemoteDataInfo($result['answer'], CURLINFO_REDIRECT_URL);
        }

        if (!empty($result) && isset($result['rawData']) && !empty($result['rawData'])) {
            $data = $result['rawData'];

            # поддержка php < 5.4. Из 5.4 удалён функционал магических ковычек и всегда возвращает false
