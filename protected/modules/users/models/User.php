...
class User extends ParentModel
{
    ...
    public function relations()
    {
        ...
        $relation['countAdRel'] = array(self::STAT, 'Apartment', 'owner_id', 'condition' => 'active = 1 AND owner_active = 1 AND deleted = 0 AND price_type IN(' . implode(", ", array_keys(HApartment::getPriceArray(Apartment::PRICE_SALE, true))) . ')', 'join' => $useIndex);
        ...
        if (issetModule('tariffPlans') && issetModule('paidservices')) {
            ...
            $relation['userTariffPlanAll'] = array(self::HAS_ONE, 'UsersTariffPlans', 'user_id', 'order' => 'userTariffPlanAll.id DESC');
        }
    ...
    public function getCountAd()
    {
        ...
        $sql = "SELECT count(id) FROM {{apartment}} WHERE active = 1 AND owner_active = 1 AND deleted = 0 AND price_type IN(" . implode(',', array_keys(HApartment::getPriceArray(Apartment::PRICE_SALE, true))) . ") AND (owner_id = :user_id OR owner_id IN (" . implode(',', $agentsId) . "))";
    ...
    public function rules()
    {
        $rules = array(
            ...
            array('username, email, phone', 'required', 'on' => 'register, register_without_confirm'),
            array('agree', 'required', 'requiredValue' => 1, 'on' => 'register, register_without_confirm',
                'message' => tc('You must agree') . ' ' . CHtml::link(tc('with User agreement'), InfoPages::getUrlById(InfoPages::LICENCE_PAGE_ID), array('target' => '_blank'))),
        );

        $rules[] = array('verifyCode', 'required', 'on' => 'register, register_without_confirm');

        $rules[] = array('verifyCode', 'CustomCaptchaValidatorFactory', 'on' => 'register, register_without_confirm');
        $rules[] = array('active, activatekey, agency_user_id, agent_status, date_created', 'safe');
        $rules[] = array('id, searchWithTariffPlan, balance', 'safe', 'on' => 'search');
    ...
    public function attributeLabels()
    {
        $return = array(
            ...
            'agree' => tc('I agree with') . ' ' . CHtml::link(tc('with User agreement'), InfoPages::getUrlById(InfoPages::LICENCE_PAGE_ID), array('target' => '_blank')),
            'searchWithTariffPlan' => tc('Tariff Plans'),
        );
...
//		}
    ...
    public static function getIdByUid($uid = false, $service = false)
    {
        ...
        if ($uid) {
            $serviceCond = '';
            if ($service) {
                $serviceCond = ' AND service = "' . $service . '" ';
            }
    ...
    public static function getModeListShow($modeList = array('block' => 1, 'table' => 1, 'map' => 1))
    {
        ...
        if (!array_key_exists($modeInState, $modeList)) {
            $modeInState = 'block';
        }
    ...
    public static function createUser($attributes, $isSocAuth = false)
    {
        ...
        if ($model->save()) {
            return array(
                ...
                'activateLink' => Yii::app()->createAbsoluteUrl('/site/activation', array('key' => $model->activatekey, 'lang' => Yii::app()->language)),
                'userModel' => $model,
            );
        } else {
            if ($isSocAuth) {
    ...
    public static function getUsersListingsIds($userId = null, $withZero = false)
    {
        if ($userId) {
            $sql = 'SELECT id FROM {{apartment}} WHERE owner_id = ' . $userId;
    ...
    public static function allowAdminEdit($user)
    {
        if ($user->role != User::ROLE_ADMIN && $user->role != User::ROLE_MODERATOR) {
            ...
            if (Yii::app()->user->id == $user->id) {
                ...
                return false;
            } elseif (Yii::app()->user->role == User::ROLE_ADMIN || ($user->role == User::ROLE_MODERATOR && $user->role != User::ROLE_MODERATOR)) {
                return true;
            } else
                return false;
        }
