...
class SearchFormModel extends ParentModel
{
    ...
    public static function getFields($isInner = false, $objType = null)
    {
        // for next version
        ...
        if (isset($objType) && $objType) {
            $searchFields = SearchFormModel::model()->sort()->findAllByAttributes(array(
                'obj_type_id' => $objType,
                'page' => $page
                ), array('group' => 'field'));
            if (!$searchFields) {
                $searchFields = SearchFormModel::model()->sort()->findAllByAttributes(array('obj_type_id' => SearchFormModel::OBJ_TYPE_ID_DEFAULT, 'page' => $page), array('group' => 'field'));
            }
        } else {
            $searchFields = SearchFormModel::model()->sort()->findAllByAttributes(array('obj_type_id' => SearchFormModel::OBJ_TYPE_ID_DEFAULT, 'page' => $page), array('group' => 'field'));
        }
