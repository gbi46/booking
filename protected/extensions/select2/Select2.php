...
class Select2 extends CInputWidget
{
    ...
    private function registerScripts($id)
    {
        ...
        $cs->registerScriptFile($this->assetsDir . $src . '/select2' . $min . '.js', CClientScript::POS_BEGIN);

        $lang = strtoupper(str_replace('_', '-', Yii::app()->language));
        $lang[0] = strtolower($lang[0]);
        $lang[1] = strtolower($lang[1]);

		$realPath = realpath(dirname(__FILE__).DIRECTORY_SEPARATOR.'assets');		
		if ($realPath && is_dir($realPath) && file_exists($realPath.$src. DIRECTORY_SEPARATOR. 'select2_locale_' . $lang . $min . '.js')) {
			$cs->registerScriptFile($this->assetsDir . $src . '/select2_locale_' . $lang . $min . '.js', CClientScript::POS_BEGIN);
		}
